...
class Card:
        ...
        if self.focus:
            if self.focus_timer > 1:
                ...
                self.image_index = int(
                    (self.focus_duration - self.focus_timer)
                    / self.focus_duration
                    * (len(self.images) - 1)
                )
    ...
    def draw(self, screen):
        ...
        screen.blit(
            ...
            (self.pos[0] - offset_x, self.pos[1] - offset_y),
        )
    ...
    def get_rect(self):
        ...
        y_offset = rect[3] / 2
        return pygame.Rect(
            self.pos[0] - x_offset, self.pos[1] - y_offset, rect[2], rect[3]
        )
    ...
    def generate_images(self, frames):
        ...
        h_card = (
            ...
            + self.keyword_labels.get_height()
            + self.margin * 3
        )
        ...
        pygame.draw.rect(
            ...
            (
                self.margin - 2,
                self.margin - 2,
                self.name_label.get_width() + 4,
                self.name_label.get_height() + 4,
            ),
            border_radius=2,
        )
        ...
        surface.blit(
            self.keyword_labels,
            (self.margin, h + self.margin * 2 + self.score_label.get_height()),
        )

        for i in range(0, frames):
            tmp_s = surface.copy()
            tmp_s = pygame.transform.scale(
                tmp_s, (w - w_decrement * i, h_card - h_decrement * i)
            )
