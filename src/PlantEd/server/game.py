...
class Game:
    def __init__(
        ...
        player_name,
        icon_name,
        level_name="spring_high_nitrate",
        start_time=0,
        resolution=3600,
        green_thumbs=25,
    ):
        since_epoch = time.time()
        self.path_to_logs = "./data/finished_games/{}{}".format(
            player_name, since_epoch
        )
    ...
    def force_end_game(self, message) -> dict:
        ...
        scoring.upload_score(
            self.player_name, self.plant.seed_mass, self.path_to_logs, self.icon_name
        )
    ...
    def update(self, message) -> dict:
        ...
        self.time += delta_t
        n_simulations = int(
            (delta_t + self.time_left_from_last_simulation) / self.resolution
        )
        self.time_left_from_last_simulation = (
            delta_t + self.time_left_from_last_simulation
        ) % self.resolution
        ...
        for i in range(n_simulations):
            ...
            percentages = {
                "leaf_percent": max(
                    (
                        growth_percentages["leaf_percent"]
                        if self.plant.get_leaf_mass_to_grow() > 0
                        else 0.1
                    ),
                    0.1,
                ),
                "stem_percent": max(
                    (
                        growth_percentages["stem_percent"]
                        if self.plant.get_stem_mass_to_grow() > 0
                        else 0.1
                    ),
                    0.1,
                ),
                "root_percent": max(
                    (
                        growth_percentages["root_percent"]
                        if self.plant.get_root_mass_to_grow() > 0
                        else 0.1
                    ),
                    0.1,
                ),
                "seed_percent": (
                    len(self.plant.seeds) * 10
                    if self.plant.get_seed_mass_to_grow() > 0
                    else 0
                ),
            ...
            sum_percentages = sum(
                [
                    ...
                    if key != "starch_percent" and key != "stomata"
                ]
            ) + max(0, percentages["starch_percent"])
        ...
        nitrate_available_mm = self.environment.nitrate_grid.available_relative_mm(
            delta_t, self.plant.root_mass, Vmax, Km, self.plant.get_root_grid()
        )
        ...
        self.log.append_model_row(
            ...
            accessible_water=self.environment.water_grid.available_absolute(
                self.plant.get_root_grid()
            ),
