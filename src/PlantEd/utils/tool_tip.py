...
class ToolTipManager:
    ...
    def update(self):
        ...
        if self.callback:
            ...
            if (
                self.tool_tips[self.current_tip].done
                and len(self.tool_tips) - 1 > self.current_tip
            ):
...
class ToolTip:
    ...
    def draw(self, screen):
        ...
        pygame.draw.rect(
            box, self.color, (0, 0, self.w, self.h), border_radius=5
        )
        init_x = self.w / 2
        ...
        screen.blit(
            self.button.image, (self.button.rect[0], self.button.rect[1])
        )
