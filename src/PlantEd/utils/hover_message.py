...
class Hover_Message:
    def __init__(self, font, line_height, margin):
        ...
        self.margin = margin
        self.hover_surface = pygame.Surface((config.SCREEN_WIDTH, config.SCREEN_HEIGHT), pygame.SRCALPHA)
        self.timer = 1
        self.w = 0
    ...
    def set_message(self, message):
        ...
        lines = message.split("*")
        self.w, h = 0, len(lines)*(self.line_height+self.margin)+self.margin
        labels = []
        ...
        pygame.draw.rect(self.hover_surface, config.WHITE, (0, 0, self.w, h), border_radius=2)
        for i in range(len(labels)):
            self.hover_surface.blit(labels[i], (self.margin,(self.line_height+self.margin)*i+self.margin))
