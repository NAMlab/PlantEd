...
class Skill:
    ...
    def draw(self, screen):
        if self.visible:
            ...
            if self.selected:
                pygame.draw.rect(
                    screen, config.WHITE, self.get_rect(), width=3
                )
            elif self.hover:
                pygame.draw.rect(
                    screen, config.GREY_TRANSPARENT, self.get_rect(), width=3
                )
...
class Skill_System:
    def __init__(
        ...
        self.margin = 20
        self.cols = cols
        self.skills = [leaf_skills, stem_skills, root_skills, starch_skills]
        for skills in self.skills:
            for skill in skills:
                skill.skills = skills
        self.active_skills = 0
        self.current_cost = 0
        self.skills_label = config.FONT.render(
            "Leaf Upgrades", False, config.BLACK
        )

        self.buy_button = Button(
            self.rect[2] - self.margin - 64,
            self.rect[3] - 30,
            64,
            64,
            [self.buy],
    ...
    def update(self, dt):
        if self.active_skills + 1 != self.plant.target_organ.type:
            if self.plant.target_organ.type == self.plant.LEAF:
                self.set_target(0)
                self.skills_label = config.BIG_FONT.render(
                    "Leaf Upgrades", True, config.BLACK
                )
            if self.plant.target_organ.type == self.plant.STEM:
                self.set_target(1)
                self.skills_label = config.BIG_FONT.render(
                    "Stem Upgrades", True, config.BLACK
                )
            if self.plant.target_organ.type == self.plant.ROOTS:
                self.set_target(2)
                self.skills_label = config.BIG_FONT.render(
                    "Root Upgrades", True, config.BLACK
                )
            if self.plant.target_organ.type == self.plant.STARCH:
                self.set_target(3)
                self.skills_label = config.BIG_FONT.render(
                    "Starch Upgrades", True, config.BLACK
                )
    ...
    def draw(self, screen):
        ...
        self.s.blit(
            self.skills_label,
            (self.rect[2] / 2 - self.skills_label.get_width() / 2, 0),
        )
        rect = (0, 45, self.rect[2], self.rect[3])
        pygame.draw.rect(
            self.s, config.WHITE_TRANSPARENT, rect, border_radius=3
        )
    ...
    def init_layout(self):
        for i in range(0, len(self.skills[0])):
            x = self.margin + (
                (64 + self.margin * 2) * (i % self.cols)
            )  # 10 for left, 20 + img_width for right
            y = self.margin + (
                (64 + self.margin / 2) * int(i / 2)
            )  # 10 for first, every row + 32 + margin
